...
const { TokenRequest: batchedTokenRequest, Client: BatchedTokensClient, BatchedTokenResponse, BatchedTokenRequest } = arbitraryBatched;
...
describe('challenge handlers', () => {
	...
	const mockPrivateKey = async (ctx: Context): Promise<CryptoKeyPair> => {
		...
		const publicKeyBase64 = Buffer.from(new Uint8Array(publicKeyRaw as ArrayBuffer)).toString("base64");
		const privateKeyBase64 = Buffer.from(new Uint8Array(privateKeyRaw as ArrayBuffer)).toString("base64");
