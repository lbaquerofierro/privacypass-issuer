...
// async function testArbitraryBatchedRequest(client, origin, issuerName: string, nTokens: number, mTLS?: MTLSConfiguration) {
...
// 	// const pk = await publicVerif.getPublicKeyBytes(issuerPublicKey);

// 	const batchedTokensClientclient = new BatchedTokensClient();
...
// 	const tokReqs = new Array<arbitraryBatched.TokenRequest>(tokChls.length);
// 	for (let i = 0; i < tokChls.length; i += 1) {
// 		const tokReq = await client.createTokenRequest(tokChls[i], issuerPublicKeyEnc);
// 		console.log('Token request', tokReq);
// 		tokReqs[i] = new TokenRequest(tokReq);
// 	}
// 	const tokenRequests = batchedTokensClientclient.createTokenRequest(tokReqs);
...
// 		},
// 		body: tokenRequests.serialize().buffer as Buffer, // node-fetch fetch requires a node.js Buffer, but ArrayBuffer seems fine
// 	});
...
// 	const tokenResponse = arbitraryBatched.BatchedTokenResponse.deserialize(
// 		new Uint8Array(await response.arrayBuffer())
// 	);
...
// 		// we see r has blindSign which is indeed a field of publicVerif.TokenResponse
// 		console.log('Token response', r);

// 		// code is failing here
// 		tokens[i] = await client.finalize(r);
// 		i += 1;
// 	}

// 	console.log('---------------------- We got the tokens ----------------------');

// 	let isValid = true;
// 	for (let i = 0; i < tokens.length; i += 1) {
// 		const token = tokens[i];
// 		isValid &&= token !== undefined && (await origin.verify(token, issuerPublicKey));
// 	}
...
async function testArbitraryBatchedRequest(issuerName: string, nTokens: number, mTLS?: MTLSConfiguration) {
	...
	for (const res of tokenResponse) {
		if (res.tokenResponse === null) {
			console.warn(`⚠️ Token Response [${j}] is null; skipping finalization.`);
			j++;
			continue;
		}
		const r = publicVerif.TokenResponse.deserialize(res.tokenResponse);
